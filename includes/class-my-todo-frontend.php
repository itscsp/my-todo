...
class MyTodoFrontend
{
    ...
    public function enqueue_frontend_scripts()
    {
        if (!is_user_logged_in()) {
            return; // Only load scripts for logged in users
        }

        wp_enqueue_script('jquery');
        wp_enqueue_style('mytodo-style', MY_TODO_PLUGIN_URL . 'assets/css/style.css');
        wp_enqueue_script('mytodo-frontend-script', MY_TODO_PLUGIN_URL . 'assets/js/frontend.js', array('jquery'), MY_TODO_VERSION, true);

        wp_localize_script('mytodo-frontend-script', 'myTodoAjax', array(
